    ...
    extends BaseGebSpec
{
    ...
    {
        given: WizardPanel wizard = contentBrowsePanel.clickToolbarNew().selectContentType( ContentTypeName.folder().toString() ).
    ...
    def "GIVEN started adding a 'Unstructured' and 'Folder' two Wizards is opened WHEN tab-menu button clicked THEN list of items with two names is present"()
    {
        given: WizardPanel wizard = contentBrowsePanel.clickToolbarNew().selectContentType( ContentTypeName.unstructured().toString() ).
            waitUntilWizardOpened();
        ...
        then: wizard.isTabMenuItemPresent( UNSTRUCTURED_TAB_MENU_ITEM ) && wizard.isTabMenuItemPresent( FOLDER_TAB_MENU_ITEM );
    ...
    {
        given: String displayName = "testname";
        ...
        when: SaveBeforeCloseDialog dialog = wizard.closeInTabMenuItem( displayName );
    ...
    {
        given: Content content = Content.builder().
