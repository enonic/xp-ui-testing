    ...
    extends BaseGebSpec
{
    ...
    {
        given: ContentPath cpath = ContentPathHelper.buildContentPath( null, REPONAME )
        BaseAbstractContent content = FolderContent.builder().withName( REPONAME ).withDisplayName( REPONAME ).withContentPath(
            cpath ).build();

        when: contentBrowsePanel.doAddContent( content, true );

        then: contentBrowsePanel.exists( content.getContentPath() );
    }
    ...
    {
        given: String name = "foldercontent";
        ...
        when: contentService.addContent( getTestSession(), content, true )
    ...
    {
        given: String name = "structuredcontent";
        ContentPath cpath = ContentPathHelper.buildContentPath( PARENT_FOLDER_ARR, name )
    ...
    {
        given: String name = "unstructuredcontent";
        ContentPath cpath = ContentPathHelper.buildContentPath( PARENT_FOLDER_ARR, name )
    ...
    {
        given: String name = "archivecontent";
        ContentPath cpath = ContentPathHelper.buildContentPath( PARENT_FOLDER_ARR, name )
    ...
    {
        given: String name = "mediacontent";
        ContentPath cpath = ContentPathHelper.buildContentPath( PARENT_FOLDER_ARR, name )
    ...
    {
        given: String name = "shortcutcontent";
        ContentPath cpath = ContentPathHelper.buildContentPath( PARENT_FOLDER_ARR, name )
    ...
    {
        given: String name = NameHelper.unqiueName( "folder" );
        ContentPath cpath = ContentPathHelper.buildContentPath( PARENT_FOLDER_ARR, name )
        BaseAbstractContent content = FolderContent.builder().withName( name ).withDisplayName( "folder" ).withContentPath( cpath ).build()

        when: contentBrowsePanel.expandContent( content.getContentPath().getParentPath() )
    ...
    def "GIVEN  existing content  WHEN content editet, name changed  THEN the content with new name should be listed beneath parent"()
    {
        ...
        when: StructuredContent newcontent = cloneContentWithNewName( contentToEdit );
        ...
        then: contentBrowsePanel.exists( newcontent.getContentPath() );
    ...
    {
        given: String name = "editdisplayname"
        ContentPath cpath = ContentPathHelper.buildContentPath( PARENT_FOLDER_ARR, name )
        StructuredContent contentToEdit = StructuredContent.builder().withName( name ).withDisplayName( name ).withContentPath(
            cpath ).build()
        contentService.addContent( getTestSession(), contentToEdit, true )

        when: StructuredContent newcontent = cloneContentWithNewDispalyName( contentToEdit )
        contentService.doOpenContentAndEdit( getTestSession(), contentToEdit, newcontent )
        contentBrowsePanel.expandContent( newcontent.getContentPath().getParentPath() );
