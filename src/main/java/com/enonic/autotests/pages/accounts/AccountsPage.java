    ...
    extends Application
{
    ...
    @FindBy(
        xpath = "//div[contains(@class,'x-toolbar-item')]//button[contains(@class,'x-btn-center') and descendant::span[contains(.,'New')]]")
    private WebElement newButtonMenu;

    public static final String ACCOUNTS_TABLE_XPATH = "//table[contains(@class,'x-grid-table')]";

    @FindBy(
        xpath = "//div[contains(@class,'x-toolbar-item')]//button[contains(@class,'x-btn-center') and descendant::span[contains(.,'Edit')]]")
    private WebElement editButton;

    @FindBy(
        xpath = "//div[contains(@class,'x-toolbar-item')]//button[contains(@class,'x-btn-center') and descendant::span[contains(.,'Delete')]]")
    private WebElement deleteButton;

    @FindBy(
        xpath = "//div[contains(@class,'x-toolbar-item')]//button[contains(@class,'x-btn-center') and descendant::span[contains(.,'Change Password')]]")
    private WebElement changePasswordButton;

    @FindBy(
        xpath = "//div[contains(@class,'x-toolbar-item')]//button[contains(@class,'x-btn-center') and descendant::span[contains(.,'open')]]")
    ...
    {
        String userDescriptionXpath =
            String.format( SYSTEMUSER_NAME_AND_DISPLAYNAME_IN_TABLE, user.getUserInfo().getDisplayName(), user.getUserInfo().getName() );
        getLogger().info( "Check is new created user present in the table: " + user.getUserInfo().getDisplayName() );

        boolean result = waitUntilVisibleNoException( By.xpath( userDescriptionXpath ), 2l );
        if ( result )
        {
            getLogger().info( "Account  was found in the Table! name:" + user.getUserInfo().getName() + " displayName:" +
                                  user.getUserInfo().getDisplayName() );
        }
        else
        {
            getLogger().info( "Account  was not found in the Table! name: " + user.getUserInfo().getName() + " displayName:" +
                                  user.getUserInfo().getDisplayName() );
        }
    ...
    public void waituntilPageLoaded( long timeout )
    {
        new WebDriverWait( getDriver(), timeout ).until(
            ExpectedConditions.visibilityOfElementLocated( By.xpath( APP_ACCOUNTS_FRAME_XPATH ) ) );
    }

    public boolean isPageLoaded()
    {
        return waitUntilVisibleNoException( By.xpath( ACCOUNTS_TABLE_XPATH ), 2l );
    }
