    ...
    extends BaseModalDialog
{
    ...
    private final String ITEMS_TO_DELETE =
        "//div[contains(@class,'admin-window')]//div[@class='delete-container']//div[@class='delete-item']//h4";

    public static final String DELETE_BUTTON_XPATH =
        "//div[contains(@class,'admin-window')]//button[contains(@class,'x-btn-center') and descendant::span[contains(.,'Delete')]]";
    ...
    public void doDelete( String displaynameToDelete )
    {
        ...
        if ( displaynameToDelete != null )
        {
            ...
            if ( elems.size() > 0 )
            {
                ...
                {
                    throw new DeleteCMSObjectException(
                        "account's display-name on the DeleteDialog not equals with expected display-name" );
                }
            }
